...
from ..builder import COMMON_HEADS
from ..losses import ranking_losses
from ..utils import vectorize_labels


@COMMON_HEADS.register_module()
class RankBasedRPNHead(AnchorHead):
    ...
    def __init__(
        self, in_channels, head_weight=0.20, rank_loss_type="RankSort", **kwargs
    ):
    ...
    def loss(self, cls_scores, bbox_preds, gt_bboxes, img_metas, gt_bboxes_ignore=None):
        ...
        if pos_idx.sum() > 0:
            ...
            flat_labels = vectorize_labels(
                cls_labels, self.num_classes, torch.cat(all_label_weights)
            )
            ...
            if self.rank_loss_type == "RankSort":
                ...
                iou_targets = bbox_overlaps(
                    pos_pred.detach(), pos_target, is_aligned=True
                )
                flat_labels[flat_labels == 1] = iou_targets
                ranking_loss, sorting_loss = self.loss_rank.apply(
                    flat_preds, flat_labels
                )

                self.sb_weight = (ranking_loss + sorting_loss).detach() / float(
                    loss_bbox.item()
                )
                loss_bbox *= self.sb_weight

                return dict(
                    loss_rpn_rank=self.head_weight * ranking_loss,
                    loss_rpn_sort=self.head_weight * sorting_loss,
                    loss_rpn_bbox=self.head_weight * loss_bbox,
                )

            elif self.rank_loss_type == "aLRP":
                e_loc = loss_bbox.detach() / (2 * (1 - 0.7))
                losses_cls, rank, order = self.loss_rank.apply(
                    flat_preds, flat_labels, e_loc
                )
                ...
                losses_bbox = (
                    torch.cumsum(ordered_losses_bbox, dim=0)
                    / rank[order.detach()].detach().flip(dims=[0])
                ).mean()
                ...
                if self.counter == self.period:
                    self.sb_weight = (
                        np.mean(self.reg_lrp_hist) + np.mean(self.cls_lrp_hist)
                    ) / np.mean(self.reg_lrp_hist)
                    self.cls_lrp_hist.clear()
                    self.reg_lrp_hist.clear()
                    self.counter = 0
                losses_bbox *= self.sb_weight
                return dict(
                    loss_rpn_cls=self.head_weight * losses_cls,
                    loss_rpn_bbox=self.head_weight * losses_bbox,
                )
