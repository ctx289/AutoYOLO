...
class RankBasedShared2FCBBoxHead(ConvFCBBoxHead):
    def __init__(
        self, fc_out_channels=1024, rank_loss_type="RankSort", *args, **kwargs
    ):
            ...
            if pos_inds.any():
                ...
                if self.rank_loss_type == "RankSort":
                    ...
                    iou_targets = bbox_overlaps(
                        pos_bbox_pred.detach(), pos_target, is_aligned=True
                    )
                    ...
                    return (
                        dict(
                            loss_roi_rank=ranking_loss,
                            loss_roi_sort=sorting_loss,
                            loss_roi_bbox=losses_bbox,
                        ),
                        bbox_weights,
                    )
                    ...
                    losses_cls, rank, order = self.loss_rank.apply(
                        flat_preds, flat_labels, loss_bbox.detach()
                    )
