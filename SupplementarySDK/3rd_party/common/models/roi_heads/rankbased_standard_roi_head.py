...
class RankBasedStandardRoIHead(StandardRoIHead):
    def forward_train(
        ...
        if self.with_mask:
            cls_loss_val = (
                bbox_results["loss_bbox"]["loss_roi_rank"].detach()
                + bbox_results["loss_bbox"]["loss_roi_sort"].detach()
            )
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels, img_metas):
        ...
        loss_bbox, bbox_weights = self.bbox_head.loss(
    ...
    def _mask_forward_train(
        ...
        if not self.share_roi_extractor:
            sampling_results, bbox_weights = self.resample_for_mask_head(
                sampling_results, bbox_weights
            )
