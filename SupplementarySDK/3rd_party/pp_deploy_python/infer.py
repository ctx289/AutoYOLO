...
class Detector(object):
    ...
    def predict_numpy(self, numpy_list):
        batch_loop_cnt = math.ceil(float(len(numpy_list)) / self.batch_size)
        results = []
        for i in range(batch_loop_cnt):
            start_index = i * self.batch_size
            end_index = min((i + 1) * self.batch_size, len(numpy_list))
            batch_numpy_list = numpy_list[start_index:end_index]
            # preprocess
            self.det_times.preprocess_time_s.start()
            inputs = self.preprocess(batch_numpy_list)
            self.det_times.preprocess_time_s.end()
            ...
            result = self.postprocess(inputs, result)
            self.det_times.postprocess_time_s.end()
            self.det_times.img_num += len(batch_numpy_list)
...
def load_predictor(model_dir,
    ...
    if device == 'GPU':
        ...
        # NOTE. Modify by ryanwfu
        config.enable_use_gpu(200, device_id)
...
def main():
    ...
    if FLAGS.video_file is not None or FLAGS.camera_id != -1:
        ...
        if FLAGS.slice_infer:
            ...
            #     numpy_list.append(im)
            # detector.predict_numpy(numpy_list)
...
"""
python3 infer.py \
    --model_dir=/youtu/xlab-team4/ryanwfu/26_real_time_scene/OUTPUT/task_YZ_BOTTLECAP1/rtdetr_r18vd_6x_bottle_cap_online_crop.yml/output_inference/rtdetr_r18vd_6x_bottle_cap_online_crop\
    --image_file=/youtu/xlab-team4/share/datasets/bottle_cap/crop_data/images/cap_inner_wall/val/脏污/1949-225.png\
    --device=GPU --threshold=0.1
"""
